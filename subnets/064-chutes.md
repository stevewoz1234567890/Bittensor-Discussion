...
- **Tempo / epoch pacing:** `360` blocks between steps; **blocks since last step:** `254`. **Emission allocation field:** `τ0.000000000` *(protocol snapshot at block 8104277)*.
- **TAO routed into swap pool reserves:** **`tao_in`** = τ206,263.398340453. **Alpha liquidity in pool (`alpha_in`)** = ‎2,629,924.954149760ش‎; **`alpha_out`** (off-pool bonded/staked tally) = ‎2,496,914.575051418ش‎.
- **Implied Alpha spot:** **`price`** τ per α unit ≈ **`τ0.078460011`** *(also **moving-average price** `0.0789821520447731` used in some dashboards)*.
- **Outstanding subnet volume accumulator:** `‎4,471,575.182213592ش‎`. **Owner hotkey / coldkey (chain):** `5F4Q3cA8EvApwhBj4eLjbb1meiLtzKgWhkF5JLRMNhRfqULc` / `5FRYKhbmfXPDoHdUUDMx27E3HuMvAzwjzFMMq3rNurUhAyS9`.
- **Subnet registered at block:** `4531295` (see explorers for approximate wall-clock age). **Is dynamic liquidity subnet:** `True`.
- **Pending emissions cues:** pending α emission `‎192.106759263ش‎`; pending root emission `τ0.000000000`.
