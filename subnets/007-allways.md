...
- **Tempo / epoch pacing:** `360` blocks between steps; **blocks since last step:** `355`. **Emission allocation field:** `τ0.000000000` *(protocol snapshot at block 8104436)*.
- **TAO routed into swap pool reserves:** **`tao_in`** = τ7,663.646388617. **Alpha liquidity in pool (`alpha_in`)** = ‎1,602,973.848010267η‎; **`alpha_out`** (off-pool bonded/staked tally) = ‎2,927,783.675229139η‎.
- **Implied Alpha spot:** **`price`** τ per α unit ≈ **`τ0.004770322`** *(also **moving-average price** `0.004767729435116053` used in some dashboards)*.
- **Outstanding subnet volume accumulator:** `‎293,990.469341231η‎`. **Owner hotkey / coldkey (chain):** `5ChTwrqwqeHpX8rLw73oDWXRjTfz83U4p3kRuKgvEfAEt8EE` / `5CAc19iETJmWD2rYVX1ht58ghCpyHq86MoBNdx5TzLfinzcx`.
- **Subnet registered at block:** `2627691` (see explorers for approximate wall-clock age). **Is dynamic liquidity subnet:** `True`.
- **Pending emissions cues:** pending α emission `‎266.033371125η‎`; pending root emission `τ0.000000000`.
- **Per-flow emission splits:** τ-in `τ0.002385160` · α-out `‎1.000000000η‎` · α-in `‎0.500000000η‎`.

#### Further numeric `DynamicInfo` fields

- **`last_step` (block):** `8104080`
- **Liquidity constant `k`:** `12284624741351378545687930739`
...
| Market cap Δ 1d | `0.729701867571663409` |
...
| --- | --- |
| NetUID (API) | `7` |
| Owner SS58 (API) | `5CAc19iETJmWD2rYVX1ht58ghCpyHq86MoBNdx5TzLfinzcx` |
| Block (API) | `8104199` |
| Time (API) | `2026-05-03T16:09:24.001Z` |
| Registration block | `2627691` |
| Registration wall time | `2024-03-24T17:51:00Z` |
